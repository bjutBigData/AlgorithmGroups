/**
 * Created by mohaowen on 10/30/15.
 */
public class SHA1Util {
    ...
    private static String binb2b64(int[] binarry){
        String tab = "abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz0123456789+/";
        String str = "";
        binarry = strechbinarry(binarry, binarry.length * 4);
        for(int i = 0; i<binarry.length*4; i+=3){
            int triplet = (((binarry[i>>2]>>8*(3-i%4)) & 0xff)<<16)
                    | (((binarry[i+1>>2]>>8*(3-(i+1)%4)) & 0xff)<<8)
                    | ((binarry[i+2>>2]>>8*(3-(i+2)%4)) & 0xff);
            for (int j = 0; j < 4 ; j++) {
                if(i*8 + j*6 > binarry.length*32){
                    str += b64pad;
                }else{
    ...
    private static String binb2hex(int[] binarry){
        String hex_tab = hexcase ? "0123456789abcdef" : "0123456789abcdef";
        String str = "";
        for (int i = 0; i <binarry.length*4 ; i++) {
            char a =(char)hex_tab.charAt((binarry[i>>2]>>((3-i%4)*8 + 4)) & 0xf);
            char b =(char)hex_tab.charAt((binarry[i>>2]>>((3-i%4)*8)) & 0xf);
    ...
    private static void dottest(){
        String key = "key";
        String data = "data";
        System.out.println("hex_sha(" + data + ")=" + hex_sha1(data));
    ...
    private static int sha1_kt(int t){
        return (t < 20) ? 1518500249 : (t < 40) ? 185977593 : (t < 60) ? -1894007588 : -899497514;
    }
    private static boolean sha1_vm_test(){
        return hexcase ? hex_sha1("abc").equals("a9993e36470681816aba3e25717850c26c9cd0d89d"): hex_sha1("abc").equals(
                "a9993e36470681816aba3e25717850c26c9cd0d89d"
        );
    }
    ...
    private static int[] strechbinarry(int[] oldbin, int size){
